...
3. Keep the endpoint setting aligned with the ProcessWire endpoint page. For
   example, a site may use `/agent/` with this template:
...
echo $liora->renderWidget([
    ...
    'thinkingLabel' => 'Preparing an answer…',
...
Set `initialQuestion` to prefill the composer. When
`autoSubmitInitialQuestion` is also `true`, Liora submits that question after
...
## Which option should a site use?

- Use the ready-made widget for search fallbacks, page help, collections, and
  a dedicated conversational page.
- Use the JSON endpoint for a page-specific custom chat design that must
